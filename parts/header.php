
<div>
  <img src="assests/images/logo/Banner[4173].jpg" style="max-height: 7.35cm;
    width: 100%;
    object-fit: initial;" />
  <!-- <div class="headercontent" >
    <h1 ><strong>Inter​national Conference on Computing​ (ICC 2019 )</strong><br /><strike>October 22-24</strike> <span>Postpond to Decemeber 10-12, 2019 - Riyadh, Saudi Arabia.
  </span></h1>

  </div> -->

</div>
<header role="banner" id="header">



   <div class="header-holder tt-logo-center">
      <div class="header-overlay">
         <div class="header-area">
            <!-- <a href="index.php" class="logo" >
              <img src="assests/images/logo/logo1.png" alt="" class="tt-retina-logo" width="81" height="50" />

              </a> -->



            <!-- ***************** - Main Menu - ***************** -->
            <nav role="navigation">
               <ul id="menu-main-nav">
                 <li>
                    <a href="index.php" class="nav-link"><span><strong>Home</strong></span></a>
                 </li>
                  <li>
                     <span><strong>Committees</strong></span>
                     <ul class="sub-menu">

                        <li><a href="General-Chairs.php"><span> General Chairs</span></a></li>
                        <li><a href="Steering-Commitee.php"><span> Steering Committee </span></a></li>
                        <li><a href="local-Organizing-Commitee.php"><span> Local Organizers </span></a></li>
                        <li><a href="Program-Commitee.php"><span> Program Committee </span></a></li>

                     </ul>
                  
                  </li>


                  <li>
                     <a href="#" class="nav-link"><span><strong>Call For Papers </strong></span></a>
                     <ul class="sub-menu">

                        <li><a href="submission.php"><span> Submission</span></a></li>
                        <li><a href="Important-Dates.php"><span> Important Dates</span></a></li>


                     </ul>
                  </li>



                  <li>
                     <a href="workshop-proposal.php"><span><strong>  Workshops </strong> </span></a>
                     <ul class="sub-menu">
                        <li><a href="Submission.php"><span> Submission</span></a></li>
                        <li><a href="Information.php"><span> Information</span></a></li>
                     </ul>
                   
                   
                  </li>

                  <li>
                     <a href="#" class="nav-link"><span><strong>Practical Information</strong></span></a>
                     <ul class="sub-menu">
                        <li><a href="http://icconf.net/registration/" target="_blank" ><span>  Registration</span></a></li>

                        <li><a href="Venue.php"><span> Venue</span></a></li>
                        <li><a href="About-Riyadh.php"><span>   About Riyadh</span></a></li>
                            <li><a href="visa_support.php"><span>   Visa Support</span></a></li>
                      <li><a href="CameraReadySubmission.php"><span>  Camera Ready Submission </span></a></li>
                     </ul>
                  </li>
                  <li>
                     <a href="Program.php" class="nav-link" ><span><strong> Program</strong></span></a>
                  </li>


                  <li>
                     <a href="Keynote-Speakers.php" class="nav-link" ><span><strong> Keynote Speakers</strong></span></a>

                  </li>
                  <!-- <li>
                     <a href="#" class="nav-link"><span><strong>   Media Center</strong></span></a>
                     <ul class="sub-menu">

                        <li>
                           <a href="News.php"><span>    News </span></a>

                        </li>

                        <li>
                           <a href="Announcements.php"><span>Announcements   </span></a>

                        </li>
                     </ul>
                  </li> -->
                  <li>
                     <a href="Sponsors-and-Partners.php"><span> <strong>Sponsors </strong> </span></a>

                  </li>
                  <li>
                     <a href="contactUs.php"><span><strong>    Contact Us</strong></span></a>

                  </li>



               </ul>
            </nav>
         </div><!-- END header-area -->
      </div><!-- END header-overlay -->
   </div><!-- END header-holder -->
</header><!-- END header -->
<!--<div>
  <img src="assests/images/logo/Banner[4173].jpg" style="max-height: 7.35cm;
    width: 100%;
    object-fit: initial;" />
  <!-- <div class="headercontent" >
    <h1 ><strong>Inter​national Conference on Computing​ (ICC 2019 )</strong><br /><strike>October 22-24</strike> <span>Postpond to Decemeber 10-12, 2019 - Riyadh, Saudi Arabia.
  </span></h1>

  </div> -->

</div>
<header role="banner" id="header">



   <div class="header-holder tt-logo-center">
      <div class="header-overlay">
         <div class="header-area">
            <!-- <a href="index.php" class="logo" >
              <img src="assests/images/logo/logo1.png" alt="" class="tt-retina-logo" width="81" height="50" />

              </a> -->



            <!-- ***************** - Main Menu - ***************** -->
            <nav role="navigation">
               <ul id="menu-main-nav">
                 <li>
                    <a href="index.php" class="nav-link"><span><strong>Home</strong></span></a>
                 </li>
                  <li>
                     <span><strong>Committees</strong></span>
                     <ul class="sub-menu">

                        <li><a href="General-Chairs.php"><span> General Chairs</span></a></li>
                        <li><a href="Steering-Commitee.php"><span> Steering Committee </span></a></li>
                        <li><a href="local-Organizing-Commitee.php"><span> Local Organizers </span></a></li>
                        <li><a href="Program-Commitee.php"><span> Program Committee </span></a></li>

                     </ul>
                  
                  </li>


                  <li>
                     <a href="#" class="nav-link"><span><strong>Call For Papers </strong></span></a>
                     <ul class="sub-menu">

                        <li><a href="submission.php"><span> Submission</span></a></li>
                        <li><a href="Important-Dates.php"><span> Important Dates</span></a></li>


                     </ul>
                  </li>



                  <li>
                     <a href="workshop-proposal.php"><span><strong>  Workshops </strong> </span></a>
                     <ul class="sub-menu">
                        <li><a href="Submission.php"><span> Submission</span></a></li>
                        <li><a href="Information.php"><span> Information</span></a></li>
                     </ul>
                   
                   
                  </li>

                  <li>
                     <a href="#" class="nav-link"><span><strong>Practical Information</strong></span></a>
                     <ul class="sub-menu">
                        <li><a href="https://app.pnu.edu.sa/ICC2019Registration/Main_Registration.aspx" target="_blank" ><span>  Registration</span></a></li>

                        <li><a href="Venue.php"><span> Venue</span></a></li>
                        <li><a href="About-Riyadh.php"><span>   About Riyadh</span></a></li>
                            <li><a href="visa_support.php"><span>   Visa Support</span></a></li>
                      <li><a href="CameraReadySubmission.php"><span>  Camera Ready Submission </span></a></li>
                     </ul>
                  </li>
                  <li>
                     <a href="Program.php" class="nav-link" ><span><strong> Program</strong></span></a>
                  </li>


                  <li>
                     <a href="Keynote-Speakers.php" class="nav-link" ><span><strong> Keynote Speakers</strong></span></a>

                  </li>
                  <!-- <li>
                     <a href="#" class="nav-link"><span><strong>   Media Center</strong></span></a>
                     <ul class="sub-menu">

                        <li>
                           <a href="News.php"><span>    News </span></a>

                        </li>

                        <li>
                           <a href="Announcements.php"><span>Announcements   </span></a>

                        </li>
                     </ul>
                  </li> -->
                  <li>
                     <a href="Sponsors-and-Partners.php"><span> <strong>Sponsors </strong> </span></a>

                  </li>
                  <li>
                     <a href="contactUs.php"><span><strong>    Contact Us</strong></span></a>

                  </li>



               </ul>
            </nav>
         </div><!-- END header-area -->
      </div><!-- END header-overlay -->
   </div><!-- END header-holder -->
</header><!-- END header -->-->
